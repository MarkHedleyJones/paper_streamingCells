...
\begin{document}
    ...
    \section{Readily Attainable Streaming Cell Efficiency} \label{sect:streamingCell}
        ...
        % \begin{equation}
        %     P_{in} = \SI{260}{kPa} \cdot 2.05\times10^{-6} \frac{m^{3}}{s} = \SI{533}{mW}
        % \end{equation}
        % \begin{equation}
        %     P_{out} = \SI{1.53}{\nano\watt}
        % \end{equation}
    ...
    \section{Sizing of Streaming Cell Harvester}
        ...
        % Setting the glass substrate at thickness \SI{500}{\micro\meter}, and using a channel depth of \SI{3.00}{\centi\meter}, this length can reduced by stacking sections of channels.
        % Stacking these cells on top of and behind each other would then result in a cube having sides of \SI{3.17}{\meter}.
        % This cube would be capable of powering the electronic water meter with a pressure differential of \SI{4.48}{\kilo\pascal}.
        % That pressure differential would be unobtainable using a domestic feed across a channel with those dimensions.


        % Now we consider a harvester with internal channel height of \SI{1.35}{\nano\metre}.
        % We have chosen this dimension as it is the midpoint between \SI{0.6}{\nano\metre} and \SI{2.1}{\nano\metre}, the optimum channel height for the conductivity of tap water.
        % At this dimension we would hope for a conversion efficiency of 1\%, half the theoretically predicted 2\%.

        % Increasing the channel's efficiency to 1\% puts its output at \SI{2.05}{\milli\watt\per\meter\per\kilo\pascal}.
        % However, reducing the channel height from \SI{75}{\micro\meter} to \SI{1.00}{\nano\meter} while preserving channel cross-section drops the power-per-width down to \SI{27.3}{\nano\watt\per\meter\per\kilo\pascal}.
        % At the target pressure of \SI{4.48}{\kilo\pascal} this delivers \SI{122}{\nano\watt\per\meter}.
        % To get \SI{5.60}{\milli\watt} would require a \SI{45.7}{\kilo\meter} wide channel.
        % Again stacking and packing the channels into a cube, the harvester would occupy a \SI{0.229}{\cubic\meter} volume.
        % This is equivalent to a cube having sides of \SI{62}{\centi\meter}.
        ...
        % Building such a device could be achieved by stacking \SI{1970000} glass channels, each of width \SI{984}{\metre} and thickness \SI{500}{\micro\metre}, creating a harvester that is \SI{0.968}{\square\kilo\metre} in size.
        % Keeping the channel length at \SI{1}{\centi\meter}, it may be possible to fit the required channels into a \SI{9680}{\cubic\meter} volume.
    ...
    \section{Conclusion}
        ...
        %Methods proposed to further increase the efficiency of streaming cell energy conversion would reduce this figure.
        ...
        % A streaming cell harvester capable of achieving the target 1\% conversion efficiency would have to operate near the theoretical maximum predicted in the literature.
        ...
        %We have estimated that harvesting 1\% of the power already lost inside a mechanical water meter would be sufficient to operate an electronic meter.

        % Based on this figure, we suggest that other methods of energy harvesting should produce more viable results.
